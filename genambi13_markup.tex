...
\author[1,$\dagger$]{Gytis Dudas}
\author[2,$\dagger$]{Greg Huber}
\author[2,3,$\ast$,$\dagger$]{Michael Wilkinson}
\author[2,$\dagger$]{David  Yllanes}
...
\corresp[$\dagger$]{Authors in alphabetical order}
...
narnavirus genomes. Applying the analysis to recent observations of \emph{Culex narnavirus 1} and \emph{Zhejiang mosquito virus 3} ambigrammatic viruses
...
functional protein. \emph{Culex narnavirus 1} has been shown to possess a second segment, also ambigrammatic, termed \lq Robin'.
We find a comparable segment for \emph{Zhejiang mosquito virus 3}, a moderately diverged relative of \emph{Culex narnavirus 1}. Our analysis of Robin polymorphisms suggests that its reverse open reading frame also does not
...
question the general hypothesis of a one-gene blueprint \MW{\citep{DeR+19,Din+20,Cep20}}.
...
\MW{non-synonymous mutations are likely to be detrimental, so that most of the observed 
...
\MW{$12$} RdRp segments of \emph{Zhejiang mosquito virus 3}, abbreviated to CNV and ZMV respectively.
...
complementary strand is non-coding. We find evidence that Robin segments are under \MW{detectable
purifying} selection. Figure \ref{fig: 1} illustrates the phylogenetic relationship of 
CNV and ZMV, and ORF-wide \MW{d$N$/d$S$} values of all their segments and coding directions 
...
degradation, but without \MW{further} evidence this requires additional hypotheses.
...
\subsection{\MW{Tests for whether the complementary strand is coding}}
\label{sec: 4.1}
...
\MW{rate $r_{\rm n}$} for transitions (${\rm A}\leftrightarrow {\rm G}$ or ${\rm C}\leftrightarrow{\rm U}$) 
\MW{is different from the rate $r_{\rm v}$ for} transversions (other single-nucleotide mutations), and negligible for
...
\MW{$N_{\rm n}$ and $N_{\rm v}$} respectively. We also determined the total number
...
was \MW{$N_{\rm sy}$} and $N_{\rm ns}$ respectively. The total number of mutations
...
\MW{$n_k^{({\rm n})}$ and $n_k^{({\rm v})}$}, and numbers of synonymous mutations
which are transitions and transversions, \MW{$s_k^{({\rm n})}$ and $s_k^{({\rm v})}$}
(with \MW{$s_k^{({\rm n})}+n_k^{({\rm n})}+s_k^{({\rm v})}+n_k^{({\rm v})}=9$}). Under the null
...
\MW{We also computed} ORF-wide d$N$/d$S$ values \MW{(plotted in figure \ref{fig: 1}({\bf b}))}, 
...
\MW{consistent with equation (\ref{eq: 5.1}). These values d$N$/d$S$ values are slightly 
...
higher for Robin sequences than it is for RdRp sequences. \MW{This may be an indication that the Robin sequence
...
\caption{\label{fig: 2}
Distribution of synonymous (blue)\MW{, non-synonymous (red) substitutions, 
...
\begin{table*}
\caption{
Nucleotide-level statistics of mutations. \MW{The consensus sequence has $N$ codons. Among the mutations 
...
\MW{The numbers of single-nucleotide synonymous and 
...
We determined the set of \MW{$N_{\rm ds}$} doubly-synonymous codons in the consensus sequence, and
...
\MW{and figure \ref{fig: 1}({\bf b})}, were
...
\MW{Recent experiments indicate that ambigrammatic narnavirus genes display unusual ribosome 
...
RNA from host defence and degradation mechanisms. \MW{We can surmise that there exists a molecule 
...
\MW{There may, however, be additional viral genes involved in ambigrammatic narnavirus 
