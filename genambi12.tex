...
question the general hypothesis of a one-gene blueprint \citep{Cep20}.
...
ZMV (\emph{Zhejiang mosquito virus 3}) (blue). {\bf b} ORF-wide d$N$/d$S$ values for forward and reverse directions of RdRp and Robin segments for both viruses.
}
\end{center}
\end{figure}
...
ORF-wide d$N$/d$S$ values were computed by assuming that every mutation in the
alignment has occurred only once to be conservative. This was motivated by the
presence of pairs of sites with four haplotypes between them ($4$G sites), an
...
\toprule
Strand&\multicolumn{1}{c}{$N$}&$M$&\multicolumn{1}{c}{$N_{\rm trn}$}&\multicolumn{1}{c}{$N_{\rm trv}$}
...
the viral RNA become stalled, creating a cover \citep{Cep20,Ret+20,Wil+21}. The ambigram property
...
alternative functions for the Robin protein may also be viral suppression of
RNA~\citep{mie_14} or formation of syncytia or viral particles.
